...
export function parseSectionFilter(raw: string | undefined): SectionFilter {
  const value = raw ?? 'all';
  if (value === 'reading' || value === 'listening' || value === 'all') return value;
  throw new ApiError(
    'INVALID_SECTION',
...
export function parseDifficultyFilter(raw: string | undefined): DifficultyFilter {
  ...
  const slugs = value.split(',').map((s) => s.trim()).filter((s) => s.length > 0);
...
export function sequenceMatchesDifficulty(sequence: number, filter: DifficultyFilter): boolean {
...
function questionLabel(q: unknown): string {
  if (isPlainObject(q)) {
    ...
    const sequence =
      typeof q.sequence === 'number' || typeof q.sequence === 'string' ? q.sequence : '?';
...
export function assertDocumentShape(raw: unknown): asserts raw is { questions: unknown[] } {
  if (!isPlainObject(raw)) {
    throw new ApiError('INVALID_FORMAT', 'Import document must be a JSON object.');
  }
  ...
  if (!Array.isArray(raw.questions)) {
    throw new ApiError('INVALID_FORMAT', 'Import document "questions" must be an array.');
  }
}
...
export function validateQuestion(raw: unknown): ExportQuestion {
  ...
  for (const opt of options) {
    ...
    if (typeof o.label !== 'string' || !OPTION_LABELS.includes(o.label as 'A')) {
      fail('Each option needs a "label" of A, B, C, or D');
    }
    if (seenLabels.has(o.label as string)) fail(`Duplicate option label "${String(o.label)}"`);
    seenLabels.add(o.label as string);
    if (typeof o.text !== 'string') fail('Each option needs a string "text"');
    if (typeof o.isCorrect !== 'boolean') fail('Each option needs a boolean "isCorrect"');
    if (o.isCorrect) correctCount += 1;
  }
  if (seenLabels.size !== 4) fail('Options must be labelled A, B, C, and D exactly once');
  ...
  if (q.section === 'reading') {
    if (q.passage !== null && q.passage !== undefined) {
      if (!isPlainObject(q.passage)) fail('"passage" must be an object or null');
      const p = q.passage as Record<string, unknown>;
      if (!isNonEmptyString(p.sourceFile)) fail('Passage needs a non-empty "sourceFile"');
    ...
    if (!Array.isArray(q.transcript)) fail('Listening question needs a "transcript" array');
    for (const seg of q.transcript as unknown[]) {
      if (!isPlainObject(seg)) fail('Each transcript segment must be an object');
      const s = seg as Record<string, unknown>;
      if (typeof s.sequence !== 'number') fail('Transcript segment needs a numeric "sequence"');
      if (typeof s.text !== 'string') fail('Transcript segment needs a string "text"');
      if (typeof s.startMs !== 'number') fail('Transcript segment needs a numeric "startMs"');
      if (typeof s.endMs !== 'number') fail('Transcript segment needs a numeric "endMs"');
    }
    if (!isPlainObject(q.audio)) fail('Listening question needs an "audio" reference');
    const a = q.audio as Record<string, unknown>;
    if (!isNonEmptyString(a.fileName)) fail('Audio reference needs a non-empty "fileName"');
  ...
  return {
    ...
    passage:
      section === 'reading' && isPlainObject(q.passage)
        ? {
            sourceFile: ((q.passage as Record<string, unknown>).sourceFile as string).trim(),
    ...
    audio:
      section === 'listening'
        ? {
            fileName: ((q.audio as Record<string, unknown>).fileName as string).trim(),
            durationMs: ((q.audio as Record<string, unknown>).durationMs as number | null) ?? null,
          }
        : null,
    transcript:
      section === 'listening'
