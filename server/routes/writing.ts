...
function requireText(req: Request): string {
  ...
  if (typeof body.text !== 'string') throw new ApiError('BAD_REQUEST', 'text must be a string.');
  return body.text;
}
...
writingRouter.post('/sessions', async (req: Request, res: Response) => {
  try {
    ...
    const input: CreateWritingSessionInput = { mode: body.mode as CreateWritingSessionInput['mode'] };
...
writingRouter.put('/sessions/:id/responses/:taskNumber', async (req: Request, res: Response) => {
  try {
    res.json(ok(await saveDraft(sessionId(req), taskNumber(req), requireText(req))));
  } catch (error) {
    handle(res, error);
  }
});
...
writingRouter.post(
  ...
  async (req: Request, res: Response) => {
    try {
      res.json(ok(await submitResponse(sessionId(req), taskNumber(req), requireText(req))));
    } catch (error) {
      handle(res, error);
    }
  },
...
writingRouter.post('/sessions/:id/correct/:taskNumber', async (req: Request, res: Response) => {
  try {
    res.json(ok(await requestCorrection(sessionId(req), taskNumber(req), requireText(req))));
  } catch (error) {
    handle(res, error);
  }
});

writingRouter.post('/sessions/:id/complete', async (req: Request, res: Response) => {
  try {
    const body = (req.body ?? {}) as Record<string, unknown>;
