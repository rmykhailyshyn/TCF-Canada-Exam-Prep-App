...
describe('parseScoreResponse', () => {
  ...
  it('throws when the score is missing or non-numeric', () => {
    expect(() => parseScoreResponse(JSON.stringify({ strengths: 'a', errors: 'b', improvements: 'c' }))).toThrow(
      ClaudeError,
    );
  });
  ...
  it('throws when a feedback field is empty', () => {
    expect(() =>
      parseScoreResponse(JSON.stringify({ score: 12, strengths: '', errors: 'b', improvements: 'c' })),
    ).toThrow(ClaudeError);
  });
});
...
describe('buildCorrectionPrompt + parseCorrectionResponse', () => {
  it('builds a prompt with the draft and parses the corrected text + suggestions', () => {
    const prompt = buildCorrectionPrompt({ prompt: 'Tâche', responseText: 'mon brouillon' });
    expect(prompt).toContain('mon brouillon');

    const parsed = parseCorrectionResponse(
      JSON.stringify({ correctedText: 'Mon brouillon corrigé.', suggestions: ['use the subjunctive', ''] }),
    );
  ...
  it('throws when suggestions is not an array', () => {
    expect(() =>
      parseCorrectionResponse(JSON.stringify({ correctedText: 'x', suggestions: 'nope' })),
    ).toThrow(ClaudeError);
  });
});
