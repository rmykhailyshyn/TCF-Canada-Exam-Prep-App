...
export async function exportQuestions(
  ...
  const rows = section === 'all'
    ? await db.select().from(questions).orderBy(asc(questions.section), asc(questions.sequence))
    : await db
        .select()
        .from(questions)
  ...
  const matched = rows.filter((q) => sequenceMatchesDifficulty(q.sequence, difficulty));

  const exported: ExportQuestion[] = [];
  if (matched.length > 0) {
    const ids = matched.map((q) => q.id);
    const passageIds = [
      ...new Set(matched.map((q) => q.passageId).filter((id): id is number => id != null)),
    ];
    ...
    const [allOptions, passageRows, audioRows, segmentRows] = await Promise.all([
      ...
      db.select().from(transcriptSegments).where(inArray(transcriptSegments.questionId, ids)),
    ]);
    ...
    for (const q of matched) {
      ...
      const segments = (segmentsByQuestion.get(q.id) ?? []).sort((a, b) => a.sequence - b.sequence);
      ...
      exported.push({
        ...
        options: (optionsByQuestion.get(q.id) ?? [])
          .map((o) => ({ label: o.label as 'A', text: o.text, isCorrect: o.isCorrect }))
        ...
        audio:
          section_ === 'listening' && audio
            ? { fileName: basename(audio.filePath), durationMs: audio.durationMs }
            : null,
        transcript:
          section_ === 'listening'
            ? segments.map((s) => ({
...
export async function importQuestions(rawDocument: unknown, override: boolean): Promise<ImportSummary> {
  ...
  await db.transaction(async (tx) => {
    for (const q of incoming) {
      const [existing] = await tx
        ...
        .from(questions)
        .where(and(eq(questions.sourceFile, q.sourceFile), eq(questions.sequence, q.sequence)));
      ...
      if (q.section === 'listening' && q.audio) {
        ...
        await tx.delete(transcriptSegments).where(eq(transcriptSegments.questionId, questionId));
        await tx.delete(audioFiles).where(eq(audioFiles.questionId, questionId));
      }
    }
  });
...
async function upsertPassage(tx: Tx, sourceFile: string, text: string): Promise<number> {
  const [existing] = await tx
...
async function upsertAudio(
  ...
  if (existing) {
    await tx.update(audioFiles).set({ filePath, durationMs }).where(eq(audioFiles.id, existing.id));
  } else {
