...
export function SpeakingSession({ session }: Props): JSX.Element {
  ...
  if (!task) return <div className="p-6 text-slate-500">No speaking tasks available.</div>;
        ...
        <div className="ml-auto flex items-center gap-4">
          {isReal && (
            <>
              ...
              {session.phaseRemainingMs !== null && session.realPhase !== 'uploading' && (
            ...
            return (
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-sm font-medium transition ${
                  i === active ? 'bg-brand-600 text-white' : 'text-slate-600 hover:bg-slate-100'
                }`}
              >
                Task {t.taskNumber}
