...
// spec: docs/specs/speaking-ui.md §Entry & mode selection (Behaviour.2–4)
// Training (untimed, sample answers + on-request correction) or Real (per-task TCF prep/record
// timing, all three tasks). In training the user practises a single task or all three.

type Props = { onStart: (config: SpeakingConfig) => void };

type TaskChoice = 'all' | 1 | 2 | 3;

export function SpeakingSetup({ onStart }: Props): JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-2xl animate-fade-in-up px-6 py-10 sm:py-14">
      <h1 className="text-3xl font-bold tracking-tight text-slate-900">Speaking · Expression orale</h1>
      <p className="mt-2 text-slate-500">
        Three spoken tasks, recorded in your browser, transcribed and scored out of 20 by the local
        Whisper + Claude CLIs.
      </p>
      ...
      <section className="mt-9">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          ...
          <Card
            ...
            icon="⏱️"
            title="Real"
            subtitle="Per-task prep + recording limits · all 3 tasks · no guidance"
          />
        </div>
      </section>
