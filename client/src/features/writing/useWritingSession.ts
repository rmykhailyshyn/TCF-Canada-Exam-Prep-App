...
export function useWritingSession(config: WritingConfig): WritingSession {
  ...
  const [evaluations, setEvaluations] = useState<
    Record<number, WritingEvaluation>
  >({});
  const [corrections, setCorrections] = useState<
    Record<number, WritingCorrection>
  >({});
      ...
      .catch((err: unknown) => {
        setError(
          err instanceof ApiError
            ? err.message
            : "Failed to start the writing session.",
        );
        setStatus("error");
      });
  }, [config]);
  ...
  const setDraft = useCallback(
    (taskNumber: number, text: string) => {
      ...
      if (saveTimers.current[taskNumber])
        clearTimeout(saveTimers.current[taskNumber]);
      saveTimers.current[taskNumber] = setTimeout(
