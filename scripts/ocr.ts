...
function mapImagesBySequence(
  ...
  for (const entry of entries) {
    ...
    if (seq == null) {
      console.warn(
        `• Ignoring image with no sequence number in its name: ${entry}`,
      );
      continue;
    }
    if (map.has(seq)) {
      console.warn(
        `• Multiple images map to question ${seq}; using ${map.get(seq)}, ignoring ${entry}`,
      );
      continue;
    }
...
async function main(): Promise<void> {
  ...
  for (const q of parsed.questions) {
    ...
    if (!imagePath) {
      console.warn(
        `• Question ${q.sequence}: no passage image found in the folder — skipping.`,
      );
      skipped += 1;
      continue;
    }
    ...
    if (existingPassage.length > 0) {
      console.warn(
        `• Passage for question ${q.sequence} already imported — skipping.`,
      );
      skipped += 1;
      continue;
    }
