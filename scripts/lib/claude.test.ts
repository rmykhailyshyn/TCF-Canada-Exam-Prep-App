...
const listening: EnrichInput = { ...reading, section: 'listening', sourceText: 'Bonjour, le train part à huit heures.' };
...
describe('parseExplanationResponse', () => {
  it('parses a clean JSON object', () => {
    expect(parseExplanationResponse(JSON.stringify(validReasons))).toEqual(validReasons);
  });
  ...
  it('throws when a reason is missing', () => {
    ...
    void correctReason;
    expect(() => parseExplanationResponse(JSON.stringify(rest))).toThrow(/correctReason/);
  });

  it('throws when a reason is empty or non-string', () => {
    expect(() => parseExplanationResponse(JSON.stringify({ ...validReasons, optionCReason: '' }))).toThrow(
      ClaudeError,
    );
    expect(() => parseExplanationResponse(JSON.stringify({ ...validReasons, optionCReason: 5 }))).toThrow(
      ClaudeError,
    );
  });
});
...
describe('parseCliEnvelope', () => {
  it('returns the result string from a success envelope', () => {
    expect(parseCliEnvelope(JSON.stringify({ is_error: false, result: '{"a":1}' }))).toBe('{"a":1}');
  });

  it('throws on a reported error', () => {
    expect(() => parseCliEnvelope(JSON.stringify({ is_error: true, result: 'rate limited' }))).toThrow(
      /rate limited/,
    );
  });

  it('throws when result is missing or output is not JSON', () => {
    expect(() => parseCliEnvelope(JSON.stringify({ is_error: false }))).toThrow(ClaudeError);
