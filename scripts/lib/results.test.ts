...
describe('crossCheckScore', () => {
  ...
  it('matches when recomputed correct/points equal the PDF summary', () => {
    const results: ParsedResults = {
      scoreSummary: { correctCount: 2, totalPoints: 12, maxPoints: 699, time: '00:10:00' },
    ...
    expect(check).toMatchObject({ recomputedCorrect: 2, recomputedPoints: 12, matches: true });
  });

  it('flags a mismatch when the recomputed score differs', () => {
    const results: ParsedResults = {
      scoreSummary: { correctCount: 2, totalPoints: 999, maxPoints: 699, time: null },
...
describe('extractSequenceFromFilename', () => {
  ...
  it('reads the number after Q, ignoring a leading test number', () => {
    expect(extractSequenceFromFilename('comprehension-ecrite-25Q39.png')).toBe(39);
