...
describe('parseSpeakingTasks', () => {
  it('parses a valid array, assigning the 0-based index as sequence', () => {
    ...
    expect(results).toEqual([
      { ok: true, task: { sequence: 0, taskNumber: 1, question: 'Présentez-vous.', sampleAnswer: 'Bonjour…' } },
      { ok: true, task: { sequence: 1, taskNumber: 3, question: 'Donnez votre avis.', sampleAnswer: null } },
    ]);
  });
  ...
  it('throws when the document is not valid JSON', () => {
    expect(() => parseSpeakingTasks('{not json')).toThrow(SpeakingTaskParseError);
  });

  it('throws when the document is not a JSON array', () => {
    expect(() => parseSpeakingTasks(JSON.stringify({ task: 1, question: 'x' }))).toThrow(
