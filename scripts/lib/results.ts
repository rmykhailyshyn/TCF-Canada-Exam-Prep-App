...
export function extractSequenceFromFilename(filename: string): number | null {
  ...
  if (numbers && numbers.length > 0) return Number.parseInt(numbers[numbers.length - 1], 10);
  return null;
}
...
export function splitStimulus(ocrText: string): { passage: string; question: string } {
  const lines = ocrText
    .split('\n')
    .map((l) => l.trim())
    .filter((l) => l.length > 0);
  ...
  const passageLines = footerIdx === -1 ? lines.slice(0, -1) : lines.slice(0, footerIdx);
  const questionLines = footerIdx === -1 ? lines.slice(-1) : lines.slice(footerIdx + 1);
