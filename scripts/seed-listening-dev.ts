...
const QUESTIONS: SeedQuestion[] = [
  ...
  },
  {
    ...
    segments: [
      ...
      { text: "Je voudrais un thé, s'il vous plaît.", startMs: 1800, endMs: 4000 },
      { text: "Très bien, j'arrive tout de suite.", startMs: 4000, endMs: 6000 },
    ],
  },
  {
    ...
    segments: [
      ...
      { text: "Le ciel sera dégagé toute la journée.", startMs: 2000, endMs: 4300 },
...
async function main(): Promise<void> {
  ...
  for (let i = 0; i < QUESTIONS.length; i += 1) {
    const path = resolve(AUDIO_DIR, `listening-dev-q${String(i + 1).padStart(2, '0')}.mp3`);
    if (!existsSync(path)) {
      throw new Error(
