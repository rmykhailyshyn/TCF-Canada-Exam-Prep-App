...
test.describe('Listening learning flow', () => {
  ...
  test('player, subtitles, answering, and results', async ({ page, listeningQuiz, results }) => {
    ...
    await expect(listeningQuiz.option(first.correct)).toBeEnabled({ timeout: 15_000 });
  ...
  test('subtitle click-to-seek moves audio position and the active highlight', async ({ listeningQuiz }) => {
    const q = await listeningQuiz.currentQuestion();
    await expect(listeningQuiz.option(q.correct)).toBeEnabled({ timeout: 15_000 });
