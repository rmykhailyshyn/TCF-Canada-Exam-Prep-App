...
export class ApiClient {
  ...
  async firstQuestionId(section: Section): Promise<number> {
    const { questions } = await this.createSession({
      section,
      mode: "learning",
      difficulty: "beginner",
    });
  ...
  async answer(
    ...
    await this.request.post(`/api/sessions/${sessionId}/answers`, {
      data: { questionId, chosenLabel },
    });
  }

  async complete(sessionId: number, elapsedMs: number): Promise<void> {
    await this.request.post(`/api/sessions/${sessionId}/complete`, {
      data: { elapsedMs },
    });
  }
