...
import { type ListeningQuestion, LISTENING_QUESTIONS } from '../fixtures/listening-data';
...
export class ListeningQuizPage {
  ...
  option(text: string): Locator {
    return this.page.getByRole('button', { name: new RegExp(`^[A-D] ${text}`) });
  }
  ...
  async currentQuestion(questions: ListeningQuestion[] = LISTENING_QUESTIONS): Promise<ListeningQuestion> {
  ...
  async audioCurrentTime(): Promise<number> {
    return this.page.evaluate(() => document.querySelector('audio')?.currentTime ?? 0);
  }
}
