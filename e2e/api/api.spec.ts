...
test.describe("Listening player endpoints", () => {
  test("GET /api/questions/:id/transcript returns ordered segments in the envelope", async ({
    ...
    const segments = body.data.segments as {
      sequence: number;
      text: string;
      startMs: number;
      endMs: number;
  ...
  test("unknown question ids return a NOT_FOUND envelope / 404", async ({
    request,
  }) => {
...
test.describe("Reading passage-image endpoint", () => {
  ...
  test("returns PASSAGE_IMAGE_NOT_FOUND for a listening question and unknown ids", async ({
    ...
    const listening = await request.get(
      `/api/questions/${listeningId}/passage-image`,
    );
...
test.describe("Question selection and session history", () => {
  ...
  test("learning-mode question order is randomized across sessions", async ({
    api,
  }) => {
  ...
  test("history list reports correct < total for a partially-correct session", async ({
    api,
  }) => {
    ...
    const { sessionId, questions } = await api.createSession({
      section: "reading",
      mode: "real",
    });
